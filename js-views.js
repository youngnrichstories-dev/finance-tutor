...
register('settings', (view) => {
  ...
  const f = h(`<div class="card"><h3 style="margin-top:0">Claude 연결 (이 기기 공통)</h3>
    ...
    <div class="field"><label>의견 받을 이메일 (💡 버튼)</label><input id="fbmail" value="${MD.esc(D.feedbackEmail || Feedback.ownerEmail())}"><div class="help">사용자들이 💡 의견 버튼에서 "이메일로 보내기"를 누르면 이 주소로 메일 앱이 열립니다. 저장된 의견 ${Feedback.items().length}개.</div></div>
