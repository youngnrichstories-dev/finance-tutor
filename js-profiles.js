  ...
  function render() {
    ...
    if (step === 1) {
      ...
      const f = h(`<div><h1>새 사용자 — 3/3 레벨 확정</h1><p class="sub">기초(1~4주) ${result.b}/4 · 비율·성장(5~6주) ${result.r}/3 · 밸류에이션·경영(7~9주) ${result.v}/3</p>
        ...
        <div id="lvs"></div>
        <div class="row spread" style="margin-top:14px"><button class="btn secondary" id="back">← 다시 풀기</button><button class="btn" id="start">이 레벨로 시작</button></div></div></div>`);
      const lvs = f.querySelector('#lvs');
      ...
      f.querySelector('#start').addEventListener('click', () => { Store.create(Object.assign({}, draft, { level: chosen, placement: { date: Store.today(), b: result.b, r: result.r, v: result.v, recommended: result.level } })); toast(`${draft.name}님, 환영합니다`); go('home'); });
      wrap.appendChild(f);
    }
  }
  render();
...
// ───────────────────────── 주차 빠른 통과 테스트 (중급·고급)
register('fastpass', (view, { week }) => {
  const w = window.CURRICULUM.find(x => x.week === week); const quiz = App.fastQuiz(week); let answered = 0, correct = 0;
  view.appendChild(h(`<div class="row spread"><span class="pill info">${week}주차 통과 테스트</span><span class="small muted">5문항 · 4개 이상 정답이면 이 주차 완료 처리</span></div><h1 style="margin-top:8px">${w.title}</h1><p class="sub">${w.goal}. 틀리면 해당 레슨 링크가 나옵니다 — 그 레슨만 읽고 다시 도전하세요.</p>`));
  ...
  function finish() {
    if (correct >= 4) { App.passWeek(week); Store.touchStreak(); res.innerHTML = `<div class="callout ceo"><b>통과 ${correct}/5</b> — ${week}주차를 완료 처리했습니다. 용어 카드는 그대로 열리니 복습은 계속하세요.</div><div class="row"><button class="btn" data-go="curriculum">커리큘럼으로</button><button class="btn secondary" data-go="home">오늘 화면으로</button></div>`; }
    else res.innerHTML = `<div class="callout mkt"><b>${correct}/5 — 아직입니다.</b> 틀린 문항의 레슨(위 링크)만 읽고 다시 도전하세요. 이 주차를 그냥 정독해도 좋습니다.</div><div class="row"><button class="btn" data-go="fastpass" data-params='${JSON.stringify({ week })}'>다시 도전</button><button class="btn secondary" data-go="curriculum">커리큘럼으로</button></div>`;
  }
});
})();
