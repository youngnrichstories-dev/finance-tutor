...
const Claude = (() => {
  ...
  async function listModels() {
    const r = await fetch(BASE + '/models?limit=50', { headers: headers() });
  ...
  async function complete({ system, messages, maxTokens = 2500, webSearch = false, temperature = 0.5 }) {
    const body = { model: model(), max_tokens: maxTokens, system, messages, temperature };
    if (webSearch) body.tools = [{ type: 'web_search_20250305', name: 'web_search', max_uses: 6 }];
    const r = await fetch(BASE + '/messages', { method: 'POST', headers: headers(), body: JSON.stringify(body) });
    if (!r.ok) {
  ...
  async function stream({ system, messages, maxTokens = 1500, temperature = 0.6, onDelta }) {
    const body = { model: model(), max_tokens: maxTokens, system, messages, temperature, stream: true };
    const r = await fetch(BASE + '/messages', { method: 'POST', headers: headers(), body: JSON.stringify(body) });
  ...
  function friendly(status, t) {
    ...
    if (status === 404) return '선택한 모델을 찾을 수 없습니다. 설정에서 모델을 다시 선택하세요.';
  ...
  return { listModels, complete, stream, model };
})();
